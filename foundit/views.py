...
def loading(request):
  ...
  job = q.enqueue(foundit.search,str(subreddit),int(postLimit),int(topComs),int(topReplies),int(topWords),int(topUsers),int(oldestPosts),int(activePosts),timeout=600)
